...
router.post('/', optionalAuth, asyncHandler(async (req, res) => {
    const { event_id, name, email, phone, company, job_title, ticket_type, custom_fields } = req.body;
    ...
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error)
        throw createError('Failed to create registration', 500);
...
router.put('/:id/status', asyncHandler(async (req, res) => {
    ...
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error)
        throw createError('Failed to update registration', 500);
    res.json(data);
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    const { error } = await supabaseAdmin
        ...
        .eq('id', id);
    if (error)
        throw createError('Failed to cancel registration', 500);
    res.status(204).send();
}));
