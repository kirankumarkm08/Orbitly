...
router.post('/', asyncHandler(async (req, res) => {
    const { name, description, thumbnail_url, html, css, components, styles, category } = req.body;
    if (!name) {
...
router.post('/:id/duplicate', asyncHandler(async (req, res) => {
    ...
    const { data, error } = await supabaseAdmin
        ...
        .single();
    if (error)
        throw createError('Failed to duplicate template', 500);
    res.status(201).json(data);
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    if (error)
        throw createError('Failed to delete template', 500);
    res.status(204).send();
}));
