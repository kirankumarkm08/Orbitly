...
router.get('/public/:slug', optionalAuth, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Event not found', 404);
  res.json(data);
}));
...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
  const { 
    name, slug, description, short_description, cover_image,
    start_date, end_date, timezone, location_type,
    venue_name, venue_address, virtual_url,
    registration_enabled, max_attendees, registration_deadline,
...
router.put('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Failed to update event', 500);
  res.json(data);
}));
...
router.delete('/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Failed to delete event', 500);
  res.status(204).send();
}));
...
router.post('/:id/publish', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Failed to publish event', 500);
  res.json(data);
}));
...
router.post('/:eventId/speakers', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Failed to add speaker to event', 500);
  res.status(201).json(data);
}));

// DELETE /api/events/:eventId/speakers/:speakerId - Remove speaker from event
router.delete('/:eventId/speakers/:speakerId', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (error) throw createError('Failed to remove speaker from event', 500);
  res.status(204).send();
}));
