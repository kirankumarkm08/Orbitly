...
const VALID_NICHES = ['ecommerce', 'events', 'launchpad', 'static'] as const;
type NicheType = typeof VALID_NICHES[number];
const DEFAULT_MODULES: Record<NicheType, string[]> = {
...
router.post('/onboarding', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!VALID_NICHES.includes(niche as NicheType)) {
  ...
  const tenantModules = DEFAULT_MODULES[niche as NicheType];
